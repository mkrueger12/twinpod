...
4. **Wait for all sub-agents to complete and synthesize findings:**
   ...
   - Verify all file paths referenced are correct (task-specific files go in `.twinpod/`)
   - Highlight patterns, connections, and architectural decisions
   - Answer the user's specific questions with concrete evidence

5. **Gather metadata for the research document:**
   - Filename: `.twinpod/TASKNAME/NN-research-DESCRIPTION.md`
     - First, ensure the artifact directory exists: `mkdir -p .twinpod` (create it at the repo root if it isn't there yet)
     - Find the task directory: `ls -La .twinpod | grep -i "eng-XXXX"`
     ...
     - Directory naming:
       - With ticket: `.twinpod/ENG-1478-parent-child-tracking/02-research-parent-child-tracking.md`
       - Without ticket: `.twinpod/authentication-flow/02-research-auth-flow.md`
...
6. **Generate research document:**
   ...
   - Write the document to `.twinpod/TASKNAME/NN-research-DESCRIPTION.md`

7. **Note GitHub permalinks (if applicable):**
   For code references in this repo (if on main or pushed):
...
8. **Respond to the user according to the template**
   ...
   - Respond following the template exactly, include GitHub permalinks if you have them.
...
The research document should read as a **technical explainer**, not a file index. A reader should be
able to understand how the system works from the document alone — file citations support the
narrative, they don't replace it. Human readability is critical, but never at the expense of
...
### Visual structure
- Use the right visual tool for the information:
  - **Tables** for comparisons, capability matrices, and structured data
  - **Mermaid diagrams** for object graphs, data flows, and architectural relationships
  - **Inline code blocks** for key type signatures, configurations, and API shapes
  - **Code examples** and **pseudocode** to illustrate patterns concretely
- Don't rely solely on bullet lists — mix prose, tables, diagrams, and code
- Technical depth and thoroughness must not be sacrificed for readability — do both
...
- **Path handling**: Task-specific research goes in `.twinpod/`
  - Use `.twinpod/ENG-XXXX-description/NN-research-DESCRIPTION.md` for task research
